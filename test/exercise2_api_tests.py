...
class MessagesTestCase (ResourcesAPITestCase):
    ...
    def test_get_messages(self):
        ...
        with resources.app.test_client() as client:
            ...
            link = data['links']
            self.assertEqual(link[0]['title'], 'Users list')
            self.assertEqual(link[0]['rel'], 'related')
            self.assertEqual(link[0]['href'],
                              resources.api.url_for(resources.Users))
            messages = data['messages']
            self.assertEqual(len(messages), initial_messages)
...
class UsersTestCase (ResourcesAPITestCase):
    ...
    def test_get_users(self):
        ...
        with resources.app.test_client() as client:
            ...
            data = json.loads(resp.data.decode("utf-8"))
            link = data['links']
            self.assertEqual(link[0]['title'], 'Messages list')
            self.assertEqual(link[0]['rel'], 'related')
            self.assertEqual(link[0]['href'], resources.api.url_for(resources.Messages))
            users = data['users']
...
class MessageTestCase (ResourcesAPITestCase):
    ...
    message_1_request = {"title":"CSS: Margin problems with IE (II)",
                 "body": "I HAVE TRIED MODIFYING THE PADDING AND IT DOES NOT WORK EITHER",
                 "sender":"AxelW"}
    messg_1_req_mod = {"title": "CSS: Margin problems with IE (II)",
                                  "body": "I have tried modifiying the \
                                   padding and it does not work either",
                                  "editor":"Admin"}
    messg_1_req_mod_wrong = {"title":"CSS: Margin problems with IE (II)",
                                        "editor":"Admin"}
    messg_1_req_wrong = {"title":"CSS: Margin problems with IE (II)"}
    message_1_response =  {"message":{
                                "title":"CSS: Margin problems with IE (II)",
                                "body":"I HAVE TRIED MODIFYING THE PADDING AND IT DOES NOT WORK EITHER",
                                "sender":{"href":"/forum/api/users/AxelW/",
                                           "rel":"author",
                                           "title":"AxelW"}
                                     },
                           "links":[{'title':'parent', 'rel':'up', 'href':'/forum/api/messages/msg-1/', 'method':'GET'},
                                     {'title':'Messages list', 'rel':'collection', 'href': '/forum/api/messages/','method':'GET'},
                                     {'href': '/forum/api/messages/msg-21/', 'rel': 'self'},
                                     {'href': '/forum/api/messages/msg-21/', 'rel': 'edit'}]
                           }
    ...
    def test_get_message(self):
        ...
        with resources.app.test_client() as client:
            ...
            link0 = links[0]
            self.assertEqual(link0['title'], 'Messages list')
            self.assertEqual(link0['rel'], 'collection')
            self.assertEqual(link0['href'], resources.api.url_for(resources.Messages))
            ...
            self.assertEqual(resources.api.url_for(resources.User,
    ...
    def test_add_message(self):
        ...
        self.assertCountEqual(self.message_1_response['links'], data['links'])
...
class UserTestCase (ResourcesAPITestCase):
    ...
    user = {"public_profile":{
                     ...
                     "restricted_profile":{"firstname":"Peter", 
                          "lastname":"DonNadie",
                          "birthday":'2010-10-01',
                          "gender":"Male",
                          "residence":"Palencia, Spain",
...
class HistoryTestCase (ResourcesAPITestCase):
    ...
    def test_get_history_number_values(self):
        ...
        with resources.app.test_client() as client:
            ...
            messages = data['messages']
            self.assertEqual(len(messages),self.messages_number)
            ...
            self.assertIn('messages',data)
            messages = data['messages']
            self.assertEqual(len(messages),self.messages2_number)
            ...
            self.assertIn('messages',data)
            messages = data['messages']
            self.assertEqual(len(messages),self.messages3_number)
    ...
    def test_get_history_timestamp_values(self):
        ...
        with resources.app.test_client() as client:
            ...
            self.assertIn('messages',data)
            messages = data['messages']
            self.assertEqual(len(messages),self.messages4_number)
            ...
            self.assertIn('messages',data)
            messages = data['messages']
            self.assertEqual(len(messages),self.messages5_number)
    ...
    def test_get_history(self):
        ...
        with resources.app.test_client() as client:
            ...
            data = json.loads(resp.data.decode("utf-8"))
            link = data['links']
            self.assertEqual(link[0]['title'], 'Sender')
            self.assertEqual(link[0]['rel'], 'parent')
            self.assertEqual(link[0]['href'], resources.api.url_for(resources.User,nickname=self.url_nickname))
            self.assertEqual(link[0]['method'], 'GET')
            self.assertEqual(link[1]['title'], 'Users')
            self.assertEqual(link[1]['rel'], 'collection')
            self.assertEqual(link[1]['href'], resources.api.url_for(resources.Users))
            self.assertEqual(link[0]['method'], 'GET')
